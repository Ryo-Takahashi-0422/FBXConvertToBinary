...
int FBXInfoManager::Init(std::string _modelPath)
{
    ...
    // materialAndTexturenameInfo用データ処理
    ...
    for (int i = 0; i < materialAndTexturenameInfo.size(); ++i)
    {
        ...
        fwrite(texturePath.c_str(), texturePathSize, 1, fp); // テクチャパス

        ++itMaterialAndTexturenameInfo;
    }

    // localPosAndRotOfMesh用のデータ処理
    ...
    // animationNameAndBoneNameWithTranslationMatrix用のデータ処理
    ...
    for (int i = 0; i < animationNameAndBoneNameWithTranslationMatrixSize; ++i)
    {
        ...
        fwrite(&tempBoneNum, sizeof(tempBoneNum), 1, fp); // ボーン数書き込み
        ...
        fwrite(&tempMatrixNum, sizeof(tempMatrixNum), 1, fp); // フレーム数書き込み
    ...
    // bonesInitialPostureMatrix用のデータ処理
